...
export class DisputesComponent extends AuthorizationToken implements OnInit {
  id: any;
  loginError: any;
  errortext: any;
  disputeList: any = [];
  ...
  listofDisputes(data: any) {
    this.mentorservice.listofDisputes(data).subscribe(
      (res: any) => {
  ...
  reply() {
    ...
    this.mentorservice.replyonDispute(body).subscribe(
      (res: any) => {
        if (res.status === "Success" && res.messageID === 200) {
          ...
          this.listofDisputes(data);
          // document.getElementById('reply')?.classList.remove('show');
